...
shuffle_runs = 5; %times the alogithm is run, one separate output for each, first level of Results
...
if SW_format == 0
    ...
    for ii = 1:size(all_files)
        ...
        for u = 1:shuffle_runs
            
            if length(stims_to_be_used) == 1
                Results{u}.(complete_data_Fall_DATA.param_table.Properties.RowNames{options.idx,1}) = Cluster_Correlations(complete_data_Fall_DATA, options);
            elseif length(stims_to_be_used) > 1
                Results{u}.(strjoin(stims_to_be_used,'_')) = Cluster_Correlations(complete_data_Fall_DATA, options);
            end
    ...
    for ii = 1:size(all_files)
        ...
        for u = 1:shuffle_runs
            if length(stims_to_be_used) == 1
                Results{u}.(complete_data_Fall_DATA.param_table.Properties.RowNames{options.idx,1}) = Cluster_Correlations(complete_data_Fall_DATA, options);
            elseif length(stims_to_be_used) > 1
                Results{u}.((strcat(strjoin(stims_to_be_used,'_'),complete_data_Fall_DATA.param_table.Properties.RowNames{options.idx,1}([end-8 : end])))) = Cluster_Correlations(complete_data_Fall_DATA, options);
            end
            ...
            if ismember(field_counter, check_list) == 1 && check_cell_counter >= minimal_neuron_number_per_subfield
                ...
                for n=1:number_of_sounds
                    ...
                    for m = 1:actual_number_of_reps
                        ...
                        shuffled_idx = length_vector(randperm(length(length_vector)));
                        ...
                        for l=1:size(traces,1)
                            ...
                            population_matrix_of_sound(l,m) = mean(traces(shuffled_idx(l),start_frame:end_frame));
                        end
                    end
                ...
                for w = 1:number_of_cells
                    for n=1:number_of_sounds
                        ...
                        for m = 1:actual_number_of_reps
                            ...
                            times_of_sounds = stim_times{randi(length(stim_times))};
