...
function image = tiff_stack_import2(tiff_fullpath, file_cnt, num_of_files)
...
fprintf('loading from stack %2d / %2d image %4d of %4d\n', file_cnt, num_of_files, 1, stack_size)
% iterate over all tif files
for ii = 1:stack_size
    fseek(fid, info(ii).StripOffsets, 'bof');
    image(:,:,ii) = fread(input_file_id, [rows columns], 'uint16=>uint16');
